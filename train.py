...
class FusionTrainer:
    ...
    def setup_data(self):
        ...
        if self.config.dataset == 'pst900':
        ...
        elif self.config.dataset in ('landslide', 'landslide_v2'):
            resolution = (self.config.img_height, self.config.img_width)
            use_v2 = (
                self.config.dataset == 'landslide_v2' or
                getattr(self.config, 'use_v2', False) or
                '1v2' in str(self.config.data_root).lower() or
                getattr(self.config, 'channels', None) is not None
            ) and (LandslideDatasetV2 is not None)

            if use_v2:
                channels = getattr(self.config, 'channels', ['rgb', 'dtm', 'slope'])
                apply_bl = getattr(self.config, 'apply_blacklist', getattr(self.config, 'apply_blaclist', True))
                bl_path = getattr(self.config, 'blacklist_path', getattr(self.config, 'blaclist_path', None))
                ignore_rgb_black = getattr(self.config, 'ignore_rgb_black', True)
                ignore_index = getattr(self.config, 'ignore_index', -100)
                pos_sampling = getattr(self.config, 'positive_aware_sampling', True)
                pos_prob = getattr(self.config, 'positive_sample_prob', 0.5)

                print(f"[INFO] Using LandslideDatasetV2 ({self.config.num_classes} classes) with channels={channels}")
                print(f"[INFO] Target Resolution: {resolution[0]}x{resolution[1]} (HxW) | Blacklist Active: {apply_bl}")
                train_dataset = LandslideDatasetV2(
                    data_dir=self.config.data_root,
                    split="train",
                    img_size=resolution,
                    channels=channels,
                    ...
                    ignore_index=ignore_index,
                    positive_aware_sampling=pos_sampling,
                    positive_sample_prob=pos_prob
                )
                val_split = getattr(self.config, 'val_source', 'val' if os.path.exists(os.path.join(self.config.data_root, 'val')) else 'test')
                val_dataset = LandslideDatasetV2(
                    data_dir=self.config.data_root,
                    split=val_split,
                    img_size=resolution,
                    channels=channels,
            ...
            raise ValueError(f"Unknown dataset: {self.config.dataset}. Choose 'landslide', 'landslide_v2', 'mfnet', or 'pst900'.")
        ...
        use_sampler = getattr(self.config, 'positive_aware_sampler', False) and self.config.dataset in ('landslide', 'landslide_v2')
        sampler = None
        shuffle = True
        if use_sampler:
        ...
        if hasattr(dataset, "samples") and len(dataset.samples) > 0:
            ...
            for s in tqdm(dataset.samples, desc="Computing class weights (raw labels)"):
                ...
                if lbl_path and os.path.exists(lbl_path):
                    ...
                    # Map to integer class (0=BG, 1=Landslide, handling 1, 255, 65535)
                    cls_mask = torch.from_numpy(((raw_mask == 1) | (raw_mask == 255) | (raw_mask == 65535)).astype(np.int64))
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (rgb, ir, masks,_) in enumerate(pbar):
            # Normalize void / padding sentinel 255 to ignore_idx (-100) only for non-landslide datasets (e.g. MFNet)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.epochs + 1):
            ...
            if self.config.dataset in ('landslide', 'landslide_v2') or self.config.num_classes == 2:
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='landslide',
        choices=['landslide', 'landslide_v2', 'mfnet', 'pst900'],
        help='Dataset selection: landslide, landslide_v2 (2 classes), mfnet (9 classes), or pst900 (5 classes)')
    parser.add_argument('--channels', type=str, default='rgb,dtm,slope',
    ...
    parser.add_argument('--blacklist_path', type=str, default='',
    ...
    parser.add_argument('--ignore_rgb_black', action=argparse.BooleanOptionalAction, default=True,
        help='Ignore RGB(0,0,0) void/border pixels in loss and evaluation metrics')
    parser.add_argument('--img_height', type=int, default=480,
        help='Input image height (default: 480 for MFNet, 720 for PST900, 512 for Landslide)')
    parser.add_argument('--img_width', type=int, default=640,
        help='Input image width (default: 640 for MFNet, 1280 for PST900, 512 for Landslide)')
